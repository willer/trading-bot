...
async def check_messages():
    ...
    if message is not None and message['type'] == 'message':
        ...
        try:
            ...
            market_position_orig = data_dict['strategy'].get(['market_position'])        # order direction: long, short, or flat
            market_position_size_orig = data_dict['strategy'].get(['market_position_size'])   # desired position after order per TV
            manual_pct = data_dict.get(['manual_pct'])

            if manual_pct is not None:
                print("manual pct override: ",manual_pct)
                if manual_pct == 0:
                    market_position_orig = "flat"
                elif manual_pct > 0:
                    market_position_orig = "long"
                else:
                    market_position_orig = "short"
